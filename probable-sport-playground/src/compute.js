...
/**
 * Runs user-provided Python code that defines `f(x)` and calls it with 2, 3, 5.
 * @param {string} code - Python code defining a function f(x)
 * @returns {Promise<number[]>} - Results of f(2), f(3), and f(5)
 */
export async function computeGraph(code) {
  const pyodide = await getPyodide();

  try {
    await pyodide.runPythonAsync(python_code);

    const result = await pyodide.runPythonAsync(`f("""${code}""")`);
